...
# Modified by Yafei Yang from https://github.com/shahaf-arica/CuVLER/blob/main/cad/modeling/roi_heads/roi_heads.py
...
ROI_MASK_HEAD_REGISTRY = Registry("ROI_MASK_HEADS")
...
def build_roi_mask_heads(cfg, input_shape):
    """
    Build ROIHeads defined by `cfg.MODEL.ROI_HEADS.NAME`.
    """
...
class Res5ROIHeads(ROIHeads):
    ...
    def from_config(cls, cfg, input_shape):
        ...
        if mask_on:
            ...
            # )
            ret["mask_head"] = build_roi_mask_heads(
                cfg,
                ShapeSpec(channels=out_channels, width=pooler_resolution, height=pooler_resolution),
            )
        return ret
...
class CustomStandardROIHeads(ROIHeads):
    ...
    def _init_mask_head(cls, cfg, input_shape):
        ...
        ret["mask_head"] = build_roi_mask_heads(cfg, shape)
        return ret
    ...
    def _forward_box(self, features: Dict[str, torch.Tensor], proposals: List[Instances]):
        ...
        if self.training:
            if self.use_droploss and not no_gt_found:
                ...
                print('weights in roi_heads', weights, weights.shape)
...
from detectron2.utils.events import get_event_storage
from detectron2.modeling.roi_heads.mask_head import BaseMaskRCNNHead, mask_rcnn_loss
    ...
    def forward(self, x, instances: List[Instances]):
        ...
        if self.training:
            if self.use_soft_targets:
                ...
                # gt_weights = cat([inst.score for inst in instances], dim=0)
